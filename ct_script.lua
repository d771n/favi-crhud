...
local cruiseEnabled, isMicOn, isFishArea = false, false, false 
...
RegisterNetEvent('FaviVehicleMileage:client:updateMelieage', function(mil, plate)
...
RegisterNetEvent('FaviSpeedHud:enteredVehicle', function(_distance, unit)
    ...
    CreateThread(function()
        while current.inVehicle do
            ...
            if not DoesEntityExist(pVehicle) or IsPedInAnyVehicle(pPed, false) == false then
                ...
                TriggerEvent('FaviSpeedHud:leftVehicle')
                break
            end
...
RegisterNetEvent('FaviSpeedHud:leftVehicle', function()
    local pPed = PlayerPedId()
...
RegisterNetEvent('FaviSpeedHud:client:isFishArea', function(boolean)
    isFishArea = boolean
end)
...
RegisterCommand("+activatecruiser", function()
    ...
    if not cruiseEnabled then
        if math.floor(cruiserSpeed * 3.6 + 0.5) >= 50 then
            ...
            qbcore.Functions.Notify('50 كم/س', "error", 5000)
        end
    else
...
RegisterNetEvent("favi-crhud:client:anchor")
AddEventHandler("favi-crhud:client:anchor", function()
    if IsPedInAnyVehicle(PlayerPedId(), true) and IsThisModelABoat(GetEntityModel(GetVehiclePedIsIn(PlayerPedId(), false))) then
        ...
        qbcore.Functions.Notify('قارب ', "error", 4000)
    end
end)

AddEventHandler('onResourceStart', function(resource)
    if resource == "favi-crhud" then
        Citizen.CreateThread(function()
            while true do
                Citizen.Wait(100)
                TriggerEvent("FaviSpeedHud:enteredVehicle")
            end
        end)
    else
        print("by Favi")
